const cars = [
  {
    ...
    qty: 0,
    imageInfor: "image/Mansory-rolls-royce-ghosts-gif.webp",
  },
  {
    id: 2,
  ...
  },
  {
    ...
    qty: 0,
    imageInfor: "image/427a6f3257d2eb85ca90330b788b204f.gif",
  },
  {
    id: 4,
    ...
    qty: 0,
    imageInfor: "image/GrayRemoteDeviltasmanian-max-1mb.gif",
  },
  {
    id: 5,
...
function render(element) {
  ...
  element.forEach((value, key) => {
    ...
    dataTable += `
    <li id="${value.id}" class="ghost-item ${value.type} ${display}">
         <div class="card">
         ...
         <button class="btn btn-info" onclick="handleInfor(this,${value.id})">More Information</button>
         </div>
    </li>
    `
  });
...
function changeTypeProduct(index) {
  ...
  let a = document.querySelector('.products-item')
  let listType = ""
  cars.forEach((value, key) => {
    if (index.id.toLocaleLowerCase().trim() == value.type.toLocaleLowerCase()) {
      ...
      listType += `
      <li id="${value.id}" class="ghost-item ${value.type}">
      <div class="card">
      <img class="ghost-image" src="${value.image}" alt="">
      <h3 class="title title-product">${value.name}</h3>
      <p class="ghost-text">${value.content}</p>
      ...
      `
      showListType.classList.add('bkgType')
      getListType.innerHTML = listType
    }
  })
...
function handleNext() {
  if (x >= 2) {
    ...
    changeBtn.classList.remove('btn')
  }
}
function handleBack() {
  if (x <= 1) {
